...
items_router = APIRouter(prefix="/api", tags=["items"])
...
def get_wants(skip: int = 0, limit: int = 20, db: Session = Depends(get_db)):
    """Get all wants (simplified endpoint for frontend)"""
    items, total = get_market_listings(
        db,
        skip=skip,
        limit=limit,
...
def get_offers(skip: int = 0, limit: int = 20, db: Session = Depends(get_db)):
    """Get all offers (simplified endpoint for frontend)"""
    items, total = get_market_listings(
        db,
        skip=skip,
        limit=limit,
