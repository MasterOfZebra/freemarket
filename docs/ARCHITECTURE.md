...
- 🔹 **Категории v6 и минимальный личный кабинет (LK)** — новая версия системы категорий с версионированием и базовый личный кабинет пользователя

## 🔄 **Категории v6 и LK** {#категории-v6-и-lk}
- Категории и терминология: введена версия категорий с таблицами `category_versions`, `categories_v6` и `category_mappings` для миграции legacy и поддержки эволюции таксономии.
- API: публичный `GET /v1/categories` с указанием версии и схемы формы; поддержка миграций объявлений в новые структуры.
- LK: минимальный личный кабинет с профилем пользователя, списком моих объявлений и активными обменами. Базовые endpoints: `/user/cabinet`, `/user/listings`, `/user/exchanges`.
- Аутентификация: JWT (короткий access token) + refresh token, хранящийся в HttpOnly, Secure cookie; ревокация токенов через Redis; rotation refresh-токенов.
- Прокси: Nginx как reverse proxy с сохранением префикса `/api` для корректной маршрутизации.
- UI: удалены дублирующиеся элементы на главной и упрощена навигация, чтобы фокусироваться на функциональности LK в будущих релизах.
