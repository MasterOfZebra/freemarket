...
**Что тестируем:** миграции версии v6 и миграцию старых объявлений в новую схему категорий, корректность обновления связей `category_mappings`.

**Данные:** несколько объявлений в старых категориях, подготовка миграций через Alembic.

**Шаги:**
- Запустите миграции через тестовую среду: `alembic upgrade head` или соответствующий скрипт `init-db`.
- Проверьте наличие таблиц: `category_versions`, `categories_v6`, `category_mappings`.
- Создайте несколько объявлений до/после миграции и убедитесь, что они сопоставляются с новыми категориями.

**Ожидаемый результат:** миграции проходят без ошибок; объявления мигрированы на корректные категории; API `/v1/categories` возвращает версию 6 и новые структуры.

---
### Test Scenario 9: Auth rotation & LK access

**Что тестируем:** безопасность JWT-потоков: rotation refresh-токенов, хранение refresh-токенов в HttpOnly, Secure cookie, ревокация в Redis, выход и внешние сессии.

**Данные:** создаются пользователи, выполняются login, refresh, logout, доступ к LK.

**Шаги:**
- Выполните вход и сохранение refresh в HttpOnly cookie.
- Выполните refresh, убедитесь что новый refresh выдается и старый помечается как отозванный.
- Сделайте logout и проверьте очистку cookie и Redis-ревокацию.
- Попробуйте повторный refresh после выхода — должно быть отклонено.

**Ожидаемый результат:** политика rotate+revoke работают, cookies помечаются как Secure/HttpOnly, Redis хранит хэш-refresh, старые токены отзываются.
